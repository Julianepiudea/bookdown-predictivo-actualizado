...
```{r pbc-distribuciones, eval=FALSE}
library(ggplot2)
library(tidyr)
...
```{r pbc-resumen-cuantitativas, eval=FALSE}
library(kableExtra)
...
```{r pbc-resumen-cualitativas, eval=FALSE}
...
```{r pbc-recodificacion, eval=FALSE}
pbc_final <- pbc_final %>%
  mutate(
    status = case_when(
      status %in% c(0, 1) ~ 0,
...
```{r pbc-km-tratamiento, eval=FALSE}
...
```{r pbc-pruebas-curvas, eval=FALSE}
