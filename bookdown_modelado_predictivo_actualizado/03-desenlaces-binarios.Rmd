...
```{r icu-imputacion-simple, eval=FALSE}
icu_simple <- icu %>%
  mutate(
...
```{r icu-densidad-imputacion, eval=FALSE}
ggplot() +
...
```{r icu-transformaciones, eval=FALSE}
icu <- icu %>%
  mutate(
    log_sys = log1p(sys)
  )
...
```{r icu-graficos-continuas, eval=FALSE}
...
```{r icu-graficos-categoricas, eval=FALSE}
